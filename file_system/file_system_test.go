	...
	for _, v := range table {
		b.Run(fmt.Sprintf("input_size_%d", v.input), func(b *testing.B) {
			for i := 0; i < 1; i++ {
				...
				buf := sequoiaTypes.NewBytesSeeker(token)
				buf2 := sequoiaTypes.NewBytesSeeker(token)

				root, _, _, err := BuildTree(buf2, 10240, 0)
				require.NoError(b, err)
...
func TestWriteFile(t *testing.T) {
	...
	buf := sequoiaTypes.NewBytesSeeker(data)
...
func TestWriteAndProveFiles(t *testing.T) {
	...
	b := sequoiaTypes.NewBytesSeeker(token)
	b2 := sequoiaTypes.NewBytesSeeker(newToken)

	root, _, _, err := BuildTree(b, chunkSize, 0)
	require.NoError(t, err)
...
func TestWriteFileWithParams(t *testing.T) {
	...
	buf := sequoiaTypes.NewBytesSeeker(data)
	buf2 := sequoiaTypes.NewBytesSeeker(data)
	buf3 := sequoiaTypes.NewBytesSeeker(data)
...
func TestBuildTree(t *testing.T) {
	...
	dataBuf := sequoiaTypes.NewBytesSeeker([]byte(data))

	root, _, _, err := BuildTree(dataBuf, 10240, sequoiaTypes.ProofTypeBlake3)
	require.NoError(t, err)
